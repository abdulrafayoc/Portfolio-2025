...
const ProjectItem = ({ image, title, location, year, counter, description }) => {
  ...
  useEffect(() => {
    ...
    hoverTimeline
      ...
      .to(
        [metaRefs.counter.current, metaRefs.title.current, metaRefs.location.current, metaRefs.year.current],
        {
          x: 0,
          opacity: 1,
          stagger: 0.1,
  ...
  return (
    <div ref={containerRef} className="item w-full h-full group relative overflow-hidden">
      <figure className="thumb w-full h-[500px] relative overflow-hidden">
        <svg
          ref={svgRef}
        ...
        >
          <p className="thumb__caption-title text-xl font-bold mb-2">{description}</p>
          <a className="thumb__caption-link text-link underline">Explore</a>
        </figcaption>
      </figure>

      <div className="item__meta mt-4 ml-4 top-0 absolute">
        <p ref={metaRefs.counter} className="item__meta-counter opacity-0 transform translate-x-5">
          {counter}
        </p>
        <h3
          ref={metaRefs.title}
